...
if [[ -n "$git_repo__add" ]];then
	for adding in "${git_repo__add[@]}";do
		#git_adding -p $(echo "$adding" | awk '{print $1}') $(echo "$adding" | awk '{print $2}')
		git_adding -p $(echo "$adding" | awk '{print $1}') $(echo "$adding" | awk '{print $2}') $(echo "$adding" | awk '{print $3}')
		echo "------------------"
	done
fi